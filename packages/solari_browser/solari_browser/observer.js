...
function installObserver(version) {
    ...
    const queryAll = (selector, root = document) => {
        ...
        for (let n = walker.nextNode(); n; n = walker.nextNode()) {
            if (n.matches(selector))
                out.push(n);
            if (n.shadowRoot)
                out.push(...queryAll(selector, n.shadowRoot));
        }
        return out;
    };
    ...
    const holds = (outer, inner) => {
        for (let n = inner; n; n = n.parentNode ?? n.host ?? null)
            if (n === outer)
                return true;
        return false;
    };
    ...
    const accessibleName = (e, seen = new Set()) => {
        ...
        const labelledBy = (e.getAttribute("aria-labelledby") ?? "").split(/\s+/).filter(Boolean)
            .map((id) => accessibleName(e.getRootNode().getElementById?.(id) ?? document.getElementById(id), seen))
            .filter(Boolean).join(" ");
        const input = e;
    ...
    const centre = (e) => {
        ...
        if (!top)
            return null;
        if (e === top || holds(e, top) || (holds(top, e) && top.tagName === "LABEL"))
            return { x, y };
    ...
    const formValues = () => queryAll("input,textarea,select")
    ...
    const textNodes = function* (root) {
        const walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT);
        for (let n = walker.nextNode(); n; n = walker.nextNode()) {
            if (n.nodeType === Node.TEXT_NODE)
                yield n;
            else if (n.shadowRoot)
                yield* textNodes(n.shadowRoot);
        }
    };
